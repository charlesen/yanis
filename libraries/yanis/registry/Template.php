...
class Template {
	...
	public function setController($_controller) {
		...
		$_SESSION ['APP_CONTROLLER'] = $_controller;
		return $this;
	}
	...
	/**
	 * Build From the Template engine
	...
	public function buildFromTemplates() {
		...
		$content = '';
		foreach ( $bits as $bit ) {
			$bitURL = TEMPLATE_BASE . '/' . $bit;
			if (file_exists ( $bitURL ) == true) {
				$content .= file_get_contents ( $bitURL );
			}
		}
	 ...
	 * @deprecated
	 */
	private function generateTemplateFile($data) {
		...
		//$contents = substr($contents, 0, -1);//Supprime la dernière virgule
		$contents .= '};';
		
		foreach ( $data as $key => $value ) {
			// Compile
			$contents .= "var " . $key . "Tpl=Hogan.compile('$value');";
			// Render
			$contents .= "var " . $key . "Out=" . $key . "Tpl.render(data);";
			// Display
			$contents .= "$('#" . $key . "_tpl').html(" . $key . "Out);";
		}
		//$contents.="document.title = titleOut;";
		

		file_put_contents ( TEMPLATE_BASE . '/Template.js', $contents );
	}
	 ...
	 * @return String
	 */
	public function renderView($view, $moduleName = null) {
		if ($moduleName !== null) {
			if (file_exists ( $this->_path ['module'] . $view )) {
				$file = $this->_path ['module'] . $view;
			}
		} else {
			if (file_exists ( $this->_path ['default'] . $view )) {
				$file = $this->_path ['default'] . $view;
			}
		}
		// Open, parse and return file
		$this->fetch ( $file );
	}
	...
	public function render() {
		...
		$content = "";
		$urlBits = explode ( '/', $this->_page->getParam ( 'page' ) );
		$pageType = array_shift ( $urlBits ); //Page or Module
		

		// Clean that stuffs
		unset ( $_SESSION ['APP_CONTROLLER'] );
		unset ( $_SESSION ['APP_MODULE'] );
		unset ( $_SESSION ['APP_ACTION'] );
