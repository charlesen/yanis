...
/*
 * @see https://developer.mozilla.org/en-US/docs/Persona/Quick_Setup (Important)
 * 
...
class Verifier {
	...
	public function verify($assertion) {
		$response = $this->_post ( array ('audience' => $this->audience, 'assertion' => $assertion ) );
	...
	protected function _post($data) {
		$ch = curl_init ();
		curl_setopt_array ( $ch, array (CURLOPT_URL => static::ENDPOINT, CURLOPT_POST => true, CURLOPT_POSTFIELDS => json_encode ( $data ), CURLOPT_HEADER => false, CURLOPT_RETURNTRANSFER => true, CURLOPT_SSL_VERIFYPEER => true, CURLOPT_SSL_VERIFYHOST => 2, CURLOPT_FOLLOWLOCATION => false, CURLOPT_HTTPHEADER => array ('Content-Type: application/json' ) ) );
