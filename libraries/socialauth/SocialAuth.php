...
class SocialAuth {
	...
	public function authenticate(\Zend_Auth_Adapter_Interface $adapter) {
		...
		if (get_class ( $identity ) !== 'SocialAuth\Auth\Identity\Generic' && ! is_subclass_of ( $identity, 'SocialAuth\Auth\Identity\Generic' )) {
			throw new \Exception ( 'Not a valid identity' );
		}
		
		$currentIdentity = $this->getIdentity ();
		
		if (false === $currentIdentity || get_class ( $currentIdentity ) !== 'SocialAuth\Auth\Identity\Container') {
